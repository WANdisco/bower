...
var readLockFile = require('../util/readLockFile');
...
Project.prototype.createLockFile = function (options,config) {

    this._logger.info('bower-lock', 'Creating lock file bower.lock');
    ...
    .spread(function (tree) {
        ...
        that.walkTree(tree, function (node) {
            ...
            that._fixNodeDependencies(node);
...
Project.prototype.install = function (decEndpoints, options, config) {
    ...
    //if production installs exact packages from bower.lock
    if(options.production){


        that._logger.info('bower-install','Installing from bower.lock');

        mout.object.forOwn(that._bowerLock.dependencies, function (lockedNode, key) {
            targets.push(lockedNode);
        });
        ...
        that._logger.info('bower-install','Installing from bower.json');
    ...
    .fin(function () {
        ...
        that._working = false;
            ...
            if (!that._options.production){
            that.createLockFile(options, config).
            then(function(){
                that.saveBowerLock();
            });
            }
...
Project.prototype.saveBowerLock = function () {
    var file;
    var jsonStr = JSON.stringify(this._bowerLock, null, '  ') + '\n';
    var jsonHash = md5(jsonStr);

    // Save only if there's something different
    if (jsonHash === this._bowerLockHash) {
        return Q.resolve();
    }

    file = this._bowerLockFile || path.join(this._config.cwd, 'bower.lock');
    return Q.nfcall(fs.writeFile, file, jsonStr)
    .then(function () {
        this._bowerLockHash = jsonHash;
        this._bowerLockFile = file;
...
Project.prototype._bootstrap = function (targets, resolved, incompatibles) {
    ...
    if (this._options.production && this._bowerLock && this._bowerLock.resolutions){
        resolutions = this._bowerLock.resolutions;
    }
    else{
...
Project.prototype._readBowerLock = function () {
    var that = this;

    if (this._bowerLock) {
        return Q.resolve(this._bowerLock);
    }

    // Read local json
    return this._bowerLock = readLockFile(path.join(that._config.cwd, 'bower.lock'), {
        //assume: { name: path.basename(this._config.cwd) || 'root' }
    })
    .spread(function (json, deprecated, assumed) {
        ...
        if (!assumed) {
            that._bowerLockFile = path.join(that._config.cwd, deprecated ? deprecated : 'bower.lock');
        }

        jsonStr = JSON.stringify(json, null, '  ') + '\n';
        that._bowerLockHash = md5(jsonStr);
        return that._bowerLock = json;
    });
};
...
Project.prototype._fixNodeDependencies = function(node){
    var that = this;
    var r={};
    var pkgMeta=node.pkgMeta;

    if (pkgMeta){
        r.target=pkgMeta.version;
        r.source=pkgMeta._source;
        r.version=pkgMeta.version;

        var resolution=undefined;


        if (pkgMeta._resolution){
            if (pkgMeta._resolution.commit)
            {
                r.type='commit';//TODO could be a branch
                r.commit=pkgMeta._resolution.commit;
                r.target=pkgMeta._resolution.commit;

            }
            //r.type=pkgMeta._resolution.type;
            r.tag=pkgMeta._resolution.tag;
            r.fixed=true;

        }
        else{
            r.release=pkgMeta._release;
        }
        ...
        if (r.commit){
            ...
            resolution=r.release;
        }
    }

    if (that._bowerLock.dependencies[node.endpoint.name] && !mout.object.equals(that._bowerLock.dependencies[node.endpoint.name],r)){
        this._logger.warn('description', 'Versions don\'t match for package '+node.endpoint.name);
    }
    else{
        that._bowerLock.dependencies[node.endpoint.name]=r;
        that._bowerLock.resolutions[node.endpoint.name]=resolution;
    }

    if (node.dependencies && mout.object.size(node.dependencies)){

        mout.object.values(node.dependencies).forEach(function (dependency) {
            that._fixNodeDependencies(dependency);
        });
    }
